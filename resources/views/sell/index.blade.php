...
@section('content')
                ...
                <table class="table table-bordered table-striped ajax_view" id="sell_table">
                    ...
                    <tfoot>
                        <tr class="bg-gray font-17 footer-total text-center"
                            style="background-color: #00434a; hover:background-color: #00363d;">
    ...
    <!-- En tu index.blade.php donde están los otros modals -->
    <div class="modal fade afip_modal" tabindex="-1" role="dialog" aria-labelledby="afipModalLabel">
    </div>
...
@section('javascript')
    ...
    <script>
        $(document).on('click', '.show-afip-modal', function(e) {
            e.preventDefault();
            const transactionId = $(this).data('transaction-id');
            const url = $(this).data('url');
        
            // Limpiar contenedor previo
            $('.afip_modal_container').empty();
        
            // Fetch del contenido
            fetch(url)
                .then(response => response.text())
                .then(html => {
                    $('.afip_modal_container').html(html);
                    
                    // Inicializar modal con BS5
                    const afipModal = new bootstrap.Modal(document.getElementById('afipModal'));
                    afipModal.show();
                    
                    // Manejar el botón enviar
                    $('#afipModal').on('click', '.btn-primary', function() {
                        const numero = $('#whatsapp_number').val();
                        const enlace = $('#afip_invoice_url').val();
                        
                        if (!/^\d{8,15}$/.test(numero)) {
                            toastr.error('Número inválido (8-15 dígitos)');
                            return;
                        }
                        
                        window.open(`https://wa.me/${numero}?text=${encodeURIComponent(`Factura: ${enlace}`)}`, '_blank');
                        afipModal.hide();
                    });
                })
                .catch(error => {
        ...
        $(document).on('hidden.bs.modal', '#afipModal', function() {
            $('.afip_modal_container').empty();
        });
    </script>
        
    <script>
        $(document).on('click', '.btn-modal[data-container=".afip_modal"]', function(e) {
            e.preventDefault();
            const url = $(this).data('href');
            
            $('.afip_modal').load(url, function() {
                // Inicializar el modal con Bootstrap 5
                const afipModal = new bootstrap.Modal($(this)[0]);
                afipModal.show();
                
                // Manejar el botón de enviar
                $(this).find('.btn-primary').off('click').on('click', function() {
                    const numero = $(this).closest('.modal').find('#whatsapp_number').val();
                    const enlace = $(this).closest('.modal').find('#afip_invoice_url').val();
                    ...
                    window.open(`https://wa.me/${numero}?text=${encodeURIComponent(enlace)}`, '_blank');
                    afipModal.hide();
                });
            });
        });
        ...
        // Limpiar al cerrar
        $(document).on('hidden.bs.modal', '.afip_modal', function() {
            $(this).empty();
        });
        ...
        $(document).on('click', '.btn-enviar-whatsapp', function() {
            ...
            // Construir mensaje
            const mensaje = `Hola! Te compartimos tu factura:\n${enlace}`;
            const urlWhatsApp = `https://wa.me/${numero}?text=${encodeURIComponent(mensaje)}`;
            
            window.open(urlWhatsApp, '_blank');
            
            // Cerrar modal
            bootstrap.Modal.getInstance(document.getElementById('afipModal')).hide();
        });
    </script>
    ...
    <script>
        $(document).on('click', '.print-invoice', function(e) {
            e.preventDefault();
            // Obtiene la URL definida en el atributo data-href del botón
            let url = $(this).data('href');
            // En lugar de usar AJAX, se abre la URL en una nueva pestaña para forzar la descarga
...
<style>
    ...
    .afip_modal_container .modal-content {
