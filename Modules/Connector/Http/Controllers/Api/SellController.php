...
require_once '/www/wwwroot/gestionenorden.com/vendor/afipsdk/afip.php/src/Afip.php';
...
class SellController extends ApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $with = ['sell_lines','sell_lines.product', 'payment_lines', 'contact'];
        $query = Transaction::where('business_id', $business_id)
                            ->where('type', 'sell');
                            
        // --- Filtro por header o query param vencimientoPago (vencimientoPago >= fecha/hora AR) ---
        $vencimientoPagoHeader = $request->header('vencimientopago') 
            ?? $request->header('VencimientoPago') 
            ?? $request->query('vencimientoPago'); 
        
        if (!empty($vencimientoPagoHeader)) {
            try {
                // Interpretar SIEMPRE en hora AR y comparar en AR (sin convertir a UTC)
                $vencimientoPagoAr = \Carbon\Carbon::parse($vencimientoPagoHeader, 'America/Argentina/Buenos_Aires');
        
                // Usar >= en lugar de >
                $query->where('transactions.vencimientoPago', '>=', $vencimientoPagoAr->format('Y-m-d H:i:s'));
            } catch (\Throwable $e) {
                \Log::warning("SellController@index: Header vencimientoPago inválido: {$vencimientoPagoHeader}. Se ignora filtro. ".$e->getMessage());
            }
        }
        
        $updatedAt = $request->header('updatedAt') ?? $request->query('updatedAt'); // permite también ?updatedAt=...
        
        if (!empty($updatedAt)) {
            try {
                // Interpretar SIEMPRE en hora AR y comparar en AR (sin convertir a UTC)
                $fechaUpdatedAr = \Carbon\Carbon::parse($updatedAt, 'America/Argentina/Buenos_Aires');
        
                // Usar >= en lugar de >
                $query->where('transactions.updated_at', '>=', $fechaUpdatedAr->format('Y-m-d H:i:s'));
            } catch (\Throwable $e) {
                \Log::warning("SellController@index: Header updatedAt inválido: {$updatedAt}. Se ignora filtro. ".$e->getMessage());
            }
        }


        
        // --- Filtro: solo registros con factura_afip no nulo si ?FacturaAfip=1 ---
        if ($request->query('FacturaAfip') == '1') {
            $query->whereNotNull('transactions.factura_afip');
        }
    ...
    public function crearFacturaApi(Request $request)
    {
        $data = $request->validate([
            'CUIT_KEY' => 'required|numeric',
            ...
            'tipo_factura' => 'required|string|in:A,B,C'
        ]);
    ...
    -----END RSA PRIVATE KEY-----";*/
            ...
            $lastVoucher = $afip->ElectronicBilling->GetLastVoucher($data['pto_vta'], $data['invoice_type']);
            $numeroFactura = $lastVoucher + 1;
    
            $ivaRates = ['iva_21' => 0.21, 'iva_27' => 0.27, 'iva_10' => 0.105, 'excento_iva' => 0];
            $valIva = $ivaRates[$data['invoice_iva']] ?? 0;
    
            $impNeto = $data['denomination_total_amount'];
            $impIVA = in_array($data['invoice_type'], [8, 13]) ? 0 : round($impNeto * $valIva, 2);
            $impTotal = $impNeto + $impIVA;
    
            $fechaServicioDesde = $data['invoice_concept'] > 1 ? intval(str_replace('-', '', $data['start_date'])) : null;
            $fechaServicioHasta = $data['invoice_concept'] > 1 ? intval(str_replace('-', '', $data['end_date'])) : null;
            $fechaVencimiento = $data['invoice_concept'] > 1 ? intval(str_replace('-', '', $data['payment_due_date'])) : null;
    
            $invoiceData = [
                'CantReg' => 1,
                'PtoVta' => $data['pto_vta'],
                'CbteTipo' => $data['invoice_type'],
                'Concepto' => $data['invoice_concept'],
                'DocTipo' => $data['buyer_document_type'],
                'DocNro' => $data['buyer_document_number'],
                'CbteDesde' => $numeroFactura,
                'CbteHasta' => $numeroFactura,
                'CbteFch' => intval(str_replace('-', '', $data['transaction_date'])),
                ...
                'ImpTrib' => 0,
                'ImpIVA' => $impIVA,
                'CondicionIVAReceptorId' => $data['iva_receptor'],
                'MonId' => 'PES',
                'MonCotiz' => 1,
            ];
            ...
            if (in_array($data['invoice_type'], [3, 8, 13])) {
                $invoiceData['CbtesAsoc'] = [[
                    'Tipo' => $data['invoice_type'] - 2,
                    'PtoVta' => $data['pto_vta'],
                    'Nro' => $data['num_factura_val']
                ]];
            }
    
            $res = $afip->ElectronicBilling->CreateVoucher($invoiceData);
            $cae = $res['CAE'];
            $caeVto = $res['CAEFchVto'];
    
            $qr = $this->generarQrAfip(
            ...
            $html = view('sell.factura', [
                ...
                'cae_due_date' => $caeVto,
                'qrImage' => $qr['base64'],
                'invoice_type' => $data['tipo_factura'], // <- USAR LETRA DIRECTA
                'recibo' => $data['invoice_type'],
            ...
            $path = storage_path('app/public/facturas_afip/');
            $filename = 'FACTURA_' . Str::slug($data['business_name']) . "_$numeroFactura.pdf";
            Browsershot::html($html)
                ->setChromePath('/opt/chrome/chrome')
            ...
            return response()->json(['error' => 'Error al generar la factura: ' . $e->getMessage()], 400);
        }
    }

    
    private function generarQrAfip($fecha, $cuit, $ptoVta, $tipoCmp, $nroCmp, $importe, $cae)
    {
        $invoice_json = [
            'ver' => 1, 'fecha' => $fecha, 'cuit' => $cuit,
            'ptoVta' => $ptoVta, 'tipoCmp' => $tipoCmp, 'nroCmp' => $nroCmp,
            'importe' => $importe, 'moneda' => 'PES', 'ctz' => 1,
            'tipoDocRec' => 80, 'nroDocRec' => 0,
            'tipoCodAut' => 'E', 'codAut' => $cae
        ];
    
        $url_qr = 'https://www.afip.gob.ar/fe/qr/?p=' . base64_encode(json_encode($invoice_json));
        return ['base64' => $url_qr];
    }
